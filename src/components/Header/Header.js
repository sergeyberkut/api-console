...
import { getCookie, launchFullScreen, cancelFullscreen } from '../../utils'

import withSendsayService from '../Hoc'
import { Logo } from '../UI'


const Header = ({ onAuthLogout, appRef }) => {
	...
	const sublogin = getCookie('sublogin')
	login += sublogin ? ' : ' : ''
		...
		<header className="header">
			<div className="header__logo-and-actions-block">
				...
				<div className="header__actions-block">
					<div className="header__current-user">{login + sublogin}</div>
					<div className="header__quit-btn" onClick={() => onAuthLogout()}>
